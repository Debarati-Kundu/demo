...
CSmooth<T>::CSmooth(float aSigma, float aPrecision)
  ...
  for (int i = 0; i <= (this->mSize >> 1); i++) {
    T aTemp = exp(i*i/(-2.0*aSigma*aSigma))/(aSigma*sqrt(2.0*NMath::Pi));
...
void boxFilterX(const CTensor<T>& aTensor, CTensor<T>& aResult, int aWidth) {
  ...
  for (int z = 0; z < aTensor.zSize(); z++)
    for (int y = 0; y < aTensor.ySize(); y++) {
      ...
      // Initialize
      T aSum = halfWidth*aTensor.data()[aOffset];
      for (int x = 0; x <= aRight; x++)
        aSum += aTensor.data()[aOffset+x];
      aSum += aDiff*aTensor.data()[aOffset+aTensor.xSize()-1];
      // Shift
      for (int x = 0; x < aTensor.xSize(); x++) {
        aResult.data()[aOffset+x] = aSum*invWidth;
        if (x-halfWidth < 0) aSum -= aTensor.data()[aOffset];
        else aSum -= aTensor.data()[aOffset+x-halfWidth];
        if (x+halfWidth+1 >= aTensor.xSize()) aSum += aTensor.data()[aOffset+aTensor.xSize()-1];
        else aSum += aTensor.data()[aOffset+x+halfWidth+1];
      }
    }
}
...
void boxFilterY(const CTensor<T>& aTensor, CTensor<T>& aResult, int aWidth) {
  ...
  if (aBottom >= aTensor.ySize()) {
    aDiff = aTensor.ySize()-1-aBottom;
    aBottom = aTensor.ySize()-1;
  }
  for (int z = 0; z < aTensor.zSize(); z++)
    for (int x = 0; x < aTensor.xSize(); x++) {
      // Initialize
      float aSum = halfWidth*aTensor(x,0,z);
      for (int y = 0; y <= aBottom; y++)
        aSum += aTensor(x,y,z);
      aSum += aDiff*aTensor(x,aTensor.ySize()-1,z);
      // Shift
      for (int y = 0; y < aTensor.ySize(); y++) {
        aResult(x,y,z) = aSum*invWidth;
        if (y-halfWidth < 0) aSum -= aTensor(x,0,z);
        else aSum -= aTensor(x,y-halfWidth,z);
        if (y+halfWidth+1 >= aTensor.ySize()) aSum += aTensor(x,aTensor.ySize()-1,z);
        else aSum += aTensor(x,y+halfWidth+1,z);
      }
    }
}
...
void boxFilterZ(const CTensor<T>& aTensor, CTensor<T>& aResult, int aWidth) {
  ...
  if (aBottom >= aTensor.zSize()) {
    aDiff = aTensor.zSize()-1-aBottom;
    aBottom = aTensor.zSize()-1;
  }
  for (int y = 0; y < aTensor.ySize(); y++)
    for (int x = 0; x < aTensor.xSize(); x++) {
      // Initialize
      float aSum = halfWidth*aTensor(x,y,0);
      for (int z = 0; z <= aBottom; z++)
        aSum += aTensor(x,y,z);
      aSum += aDiff*aTensor(x,y,aTensor.zSize()-1);
      // Shift
      for (int z = 0; z < aTensor.zSize(); z++) {
        aResult(x,y,z) = aSum*invWidth;
        if (z-halfWidth < 0) aSum -= aTensor(x,y,0);
        else aSum -= aTensor(x,y,z-halfWidth);
        if (z+halfWidth+1 >= aTensor.zSize()) aSum += aTensor(x,y,aTensor.zSize()-1);
        else aSum += aTensor(x,y,z+halfWidth+1);
      }
    }
}
