...
public:
  ...
  // Copies a box from the tensor into aResult, the size of aResult will be adjusted
  void cut(CTensor<T>& aResult, int x1, int y1, int z1, int x2, int y2, int z2, int a);
...
// cut
template <class T>
void CTensor4D<T>::cut(CTensor<T>& aResult, int x1, int y1, int z1, int x2, int y2, int z2, int a) {
  aResult.setSize(x2-x1+1,y2-y1+1,z2-z1+1);
  for (int z = z1; z <= z2; z++)
    for (int y = y1; y <= y2; y++)
      for (int x = x1; x <= x2; x++)
	aResult(x-x1,y-y1,z-z1) = operator()(x,y,z,a);
}
